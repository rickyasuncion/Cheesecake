...
const MovieCard = ({ id, media_type, title, name, poster_path, showFavButton = true, className }) => {
  ...
  async function getTrailer(movieId) {
    try {
      ...
      if (data.results) {
        ...
        setTrailerPath(null); // No results, so no trailer available
      }
    } catch (error) {
      console.error("Error fetching trailer:", error);
      setTrailerPath(null); // Handle fetch error by setting trailer path to null
    }
  }
  ...
  useEffect(() => {
    ...
    const isAlreadyFavourite = favouriteMovies?.find(movie_id => movie_id === id) || favouriteTv?.find(tvId => tvId === id);
